...
func newExecuteTool() mcp.Tool {
	return mcp.NewTool("db_execute",
		mcp.WithDescription("Run write SQL on a source in rw mode, such as INSERT, UPDATE, or DELETE. If you only need to read data, use db_query instead. Call db_help first if you are not sure whether the source allows writes."),
		mcp.WithString("source",
			mcp.Description("Database source key. The source must be rw. Call db_help first to see available sources."),
			mcp.Required(),
		),
		mcp.WithString("sql",
...
func handleExecute(pool *Pool, logger *sqllog.Logger) func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if src.Mode == ModeR {
			errMsg = fmt.Sprintf("source=%s is read-only and does not allow db_execute", src.Key)
			return renderErrorf("%s", errMsg), nil
		}

		if err := AllowWriteSQL(sqlText); err != nil {
			errMsg = fmt.Sprintf("source=%s: %v", src.Key, err)
			return renderErrorf("%s", errMsg), nil
		}
		...
		if err != nil {
			errMsg = fmt.Sprintf("execute failed [%s]: %v", src.Key, err)
			return renderErrorf("%s", errMsg), nil
		}
