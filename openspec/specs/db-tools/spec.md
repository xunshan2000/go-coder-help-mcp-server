# db-tools Specification

## Purpose
定义通过 MCP 协议暴露的数据库工具集（`db_list_tables` / `db_describe_table` / `db_query` / `db_execute`）及其方言无关的 Driver 接口约束、只读保护、参数化传递与查询超时等行为，确保 MCP 客户端可在多数据源、多数据库方言场景下安全执行只读查询与受控写入。
